...
class Orders extends BaseModel
{
    ...
    //orders count for the listing section
    public function getOrderCounts()
    {
        $counts = array('all' => 0, 'completed' => 0, 'processing' => 0, 'refunded' => 0, 'failed' => 0,'pending' => 0,'cancelled' => 0,'on-hold' => 0, 'trash' => 0 );

        $all = Orders::all('id')->count();
        $counts['all'] = $all ;

        $query = Orders::query('');
        $query->where('orders.status', '=', 'completed');
        $counts['completed'] = $query->count();

        $query = Orders::query('');
        $query->where('orders.status', '=', 'processing');
        $counts['processing'] = $query->count();

        $query = Orders::query('');
        $query->where('orders.status', '=', 'refunded');
        $counts['refunded'] = $query->count();

        $query = Orders::query('');
        $query->where('orders.status', '=', 'failed');
        $counts['failed'] = $query->count();

        $query = Orders::query('');
        $query->where('orders.status', '=', 'pending');
        $counts['pending'] = $query->count();

        $query = Orders::query('');
        $query->where('orders.status', '=', 'cancelled');
        $counts['cancelled'] = $query->count();

        $query = Orders::query('');
        $query->where('orders.status', '=', 'on-hold');
        $counts['on-hold'] = $query->count();

        $query = Orders::query('');
        $query->where('orders.status', '=', 'deleted');
        $counts['trash'] = $query->count();
        return $counts;
